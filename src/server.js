...
import { scrapeBgJargon } from "./scraper.js";
import {
  validateWord,
  createErrorResponse,
...
app.get("/api", async (req, res) => {
  try {
    ...
    const result = await scrapeBgJargon(word);

    if (!result) {
      return res
        .status(404)
        .json(createErrorResponse(`No definitions found for "${word}"`, 404));
    }
...
app.listen(PORT, () => {
  ...
  console.log(`🔍 Example: http://localhost:${PORT}/api?word=лани`);
});
